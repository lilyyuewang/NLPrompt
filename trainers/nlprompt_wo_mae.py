...
class NLPrompt(TrainerX):
    def __init__(self, cfg):
        super().__init__(cfg)
        # self.GCE_loss = GeneralizedCrossEntropy(q=1.0)  # Commented out: MAE loss removed
        self.num_equal = []
        self.confident_rate = []
        self.clean_rate  = []
    ...
    def after_epoch(self):
        ...
        if self.cfg.DATASET.USE_OT == True:
            ...
            # self.train_loader_u = copy.deepcopy(self.tmp_train_loader_x)  # Commented out: MAE training removed
            print("after epoch: len(clean dataset)", len(self.train_loader_x.dataset.data_source))
            # print("after epoch: len(noisy dataset)", len(self.train_loader_u.dataset.data_source))  # Commented out: MAE training removed
